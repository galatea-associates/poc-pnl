...
public class PnLService {
  ...
  public void calculateEODPnL(LocalDate eodDate) {
    ...
    for (String book : books) {
      ...
      for (String instrument : instruments) {
        ...
        // value positions
        // TODO: this needs a date component eventually...
        ...
        for (Position position : positions) {
          ...
          // TODO: this is really calculating UNREALIZED P&L
          PnL pnlResult = calculatePnl(currentValuation, referenceValuation, referenceValuationKey);
          log.info("P&L result: {}", pnlResult);

          // TODO: we should combine the unrealized+realized PnL into one record for saving
          persistPnL(pnlResult);
        }

        // TODO: now that we have the positional component of P&L, need the realized P&L (from trades made
  ...
  // get all instruments this book has either a position in on the eodDate, or a trade after the
  ...
  private PnL calculatePnl(Valuation currentValuation, Valuation referenceValuation, ValuationKey valuationReferenceKey) {
    ...
    return PnL.builder().book(valuationReferenceKey.getBook()).instrument(valuationReferenceKey.getInstrument()).date(LocalDate.now())
        .referenceValuation(referenceValuation).currentValuation(currentValuation).pnl(pnl).build();
  }
